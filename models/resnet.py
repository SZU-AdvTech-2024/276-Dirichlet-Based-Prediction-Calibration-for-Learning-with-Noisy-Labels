...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor, weights=None, get_feat = False) -> Tensor:
        if weights == None:
            ...
            x = F.batch_norm(x, self.bn1.running_mean, self.bn1.running_var, weights['bn1.weight'], weights['bn1.bias'],
                             training=True)
            x = F.threshold(x, 0, 0, inplace=True)
            x = F.max_pool2d(x, kernel_size=3, stride=2, padding=1)
            residual = x
            # import ipdb; ipdb.set_trace()
            out = F.conv2d(x, weights['layer1.%d.conv1.weight' % 0], stride=1, padding=1)
            out = F.batch_norm(out, self.layer1[0].bn1.running_mean, self.layer1[0].bn1.running_var,
                                weights['layer1.%d.bn1.weight' % 0], weights['layer1.%d.bn1.bias' % 0],
                                training=True)
            out = F.threshold(out, 0, 0, inplace=True)
            out = F.conv2d(out, weights['layer1.%d.conv2.weight' % 0], stride=1, padding=1)
            out = F.batch_norm(out, self.layer1[0].bn2.running_mean, self.layer1[0].bn2.running_var,
                                weights['layer1.%d.bn2.weight' % 0], weights['layer1.%d.bn2.bias' % 0],
                                training=True)
            out = F.conv2d(x, weights['layer1.%d.conv1.weight' % 1], stride=1, padding=1)
            out = F.batch_norm(out, self.layer1[1].bn1.running_mean, self.layer1[1].bn1.running_var,
                                weights['layer1.%d.bn1.weight' % 1], weights['layer1.%d.bn1.bias' % 1],
                                training=True)
            out = F.threshold(out, 0, 0, inplace=True)
            out = F.conv2d(out, weights['layer1.%d.conv2.weight' % 1], stride=1, padding=1)
            out = F.batch_norm(out, self.layer1[1].bn2.running_mean, self.layer1[1].bn2.running_var,
                                weights['layer1.%d.bn2.weight' % 1], weights['layer1.%d.bn2.bias' % 1],
                                training=True)
            # layer 1
                # out = F.threshold(out, 0, 0, inplace=True)
                ...
                #                    weights['layer1.%d.bn3.weight' % i], weights['layer1.%d.bn3.bias' % i],
                #                    training=True)
                # if i == 0:
                #     residual = F.conv2d(x, weights['layer1.%d.downsample.0.weight' % i], stride=1)
                #     residual = F.batch_norm(residual, self.layer1[i].downsample[1].running_mean,
                #                             self.layer1[i].downsample[1].running_var,
                #                             weights['layer1.%d.downsample.1.weight' % i],
                #                             weights['layer1.%d.downsample.1.bias' % i], training=True)
            x = out + residual
            x = F.threshold(x, 0, 0, inplace=True)
            # layer 2
            for i in range(2):
                residual = x
                if i == 0:
                    out = F.conv2d(x, weights['layer2.%d.conv1.weight' % i], stride=2, padding=1)
                else:
                ...
                out = F.batch_norm(out, self.layer2[i].bn1.running_mean, self.layer2[i].bn1.running_var,
                                   weights['layer2.%d.bn1.weight' % i], weights['layer2.%d.bn1.bias' % i],
                                   training=True)
                out = F.threshold(out, 0, 0, inplace=True)
                ...
                out = F.batch_norm(out, self.layer2[i].bn2.running_mean, self.layer2[i].bn2.running_var,
                                   weights['layer2.%d.bn2.weight' % i], weights['layer2.%d.bn2.bias' % i],
                                   training=True)
                out = F.threshold(out, 0, 0, inplace=True)
                ...
                #                    weights['layer2.%d.bn3.weight' % i], weights['layer2.%d.bn3.bias' % i],
                #                    training=True)
                if i == 0:
                    residual = F.conv2d(x, weights['layer2.%d.downsample.0.weight' % i], stride=2)
                    residual = F.batch_norm(residual, self.layer2[i].downsample[1].running_mean,
                                            self.layer2[i].downsample[1].running_var,
                                            weights['layer2.%d.downsample.1.weight' % i],
                                            weights['layer2.%d.downsample.1.bias' % i], training=True)
                x = out + residual
                x = F.threshold(x, 0, 0, inplace=True)
            # layer 3
            for i in range(2):
                residual = x   
                if i==0:               
                    out = F.conv2d(x, weights['layer3.%d.conv1.weight' % i], stride=2, padding=1)
                else:
                ...
                out = F.batch_norm(out, self.layer3[i].bn1.running_mean, self.layer3[i].bn1.running_var,
                                   weights['layer3.%d.bn1.weight' % i], weights['layer3.%d.bn1.bias' % i],
                                   training=True)
                out = F.threshold(out, 0, 0, inplace=True)
                ...
                out = F.batch_norm(out, self.layer3[i].bn2.running_mean, self.layer3[i].bn2.running_var,
                                   weights['layer3.%d.bn2.weight' % i], weights['layer3.%d.bn2.bias' % i],
                                   training=True)
                out = F.threshold(out, 0, 0, inplace=True)
                ...
                #                    weights['layer3.%d.bn3.weight' % i], weights['layer3.%d.bn3.bias' % i],
                #                    training=True)
                if i == 0:
                    residual = F.conv2d(x, weights['layer3.%d.downsample.0.weight' % i], stride=2)
                    residual = F.batch_norm(residual, self.layer3[i].downsample[1].running_mean,
                                            self.layer3[i].downsample[1].running_var,
                                            weights['layer3.%d.downsample.1.weight' % i],
                                            weights['layer3.%d.downsample.1.bias' % i], training=True)
                x = out + residual
                x = F.threshold(x, 0, 0, inplace=True)
            # layer 4
            for i in range(2):
                residual = x
                if i==0:                
                    out = F.conv2d(x, weights['layer4.%d.conv1.weight' % i], stride=2, padding=1)
                else:
                ...
                out = F.batch_norm(out, self.layer4[i].bn1.running_mean, self.layer4[i].bn1.running_var,
                                   weights['layer4.%d.bn1.weight' % i], weights['layer4.%d.bn1.bias' % i],
                                   training=True)
                out = F.threshold(out, 0, 0, inplace=True)
                ...
                out = F.batch_norm(out, self.layer4[i].bn2.running_mean, self.layer4[i].bn2.running_var,
                                   weights['layer4.%d.bn2.weight' % i], weights['layer4.%d.bn2.bias' % i],
                                   training=True)
                out = F.threshold(out, 0, 0, inplace=True)
                ...
                #                    weights['layer4.%d.bn3.weight' % i], weights['layer4.%d.bn3.bias' % i],
                #                    training=True)
                if i == 0:
                    residual = F.conv2d(x, weights['layer4.%d.downsample.0.weight' % i], stride=2)
                    residual = F.batch_norm(residual, self.layer4[i].downsample[1].running_mean,
                                            self.layer4[i].downsample[1].running_var,
                                            weights['layer4.%d.downsample.1.weight' % i],
                                            weights['layer4.%d.downsample.1.bias' % i], training=True)
                x = out + residual
                x = F.threshold(x, 0, 0, inplace=True)
    ...
    def forward(self, x: Tensor, weights=None, get_feat = False) -> Tensor:
        return self._forward_impl(x, weights, get_feat)
...
def resnet18(
    ...
    """
    return _resnet('resnet18', PreActBlock, [2, 2, 2, 2], pretrained, progress,
