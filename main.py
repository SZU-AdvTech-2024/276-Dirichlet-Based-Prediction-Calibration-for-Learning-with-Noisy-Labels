...
parser.add_argument('--lambda_u', type=float, default=0)
parser.add_argument('--warmup_epoch', type=int, default=10)
parser.add_argument('--need_clean', type=bool, default=False)
parser.add_argument('--single_meta', type=int, default=0)
...
def main():
    ...
    config['lambda_u'] = args.lambda_u
    config['warmup_epoch'] = args.warmup_epoch
    config['single_meta'] = args.single_meta
    config['need_clean'] = args.need_clean
    print_config(config)
    ...
    elif config['algorithm'] == 'DPC':
        model = algorithms.DPC(config,
                                input_channel=config['input_channel'],
                                num_classes=config['num_classes'])
        # model2 = algorithms.DPC(config,
        #                         input_channel=config['input_channel'],
        #                         num_classes=config['num_classes'])
        train_mode = 'train'

    elif config['algorithm'] == 'DPC_L2B':
        model = algorithms.DPC_L2B(config,
                                input_channel=config['input_channel'],
                                num_classes=config['num_classes'])
        train_mode = 'train'
    ...
    if 'cifar' in config['dataset']:
        if config['algorithm'] == 'DPC':
            ...
            dataloaders = cifar_dataloader(cifar_type=config['dataset'],
                                           ...
                                           noise_type=config['noise_type'],
                                           percent=config['percent'])
            testloader, metaloader= dataloaders.run(
                mode='test'), dataloaders.run(mode='meta')
    ...
    for epoch in range(start_epoch, config['epochs']):
        # train
        if config['algorithm'] == 'DPC':
            model.train(epoch, dataloaders)
            #model2.train(epoch, model1.model)
        elif config['algorithm'] == 'DPC_L2B':
            model.train(epoch, dataloaders, metaloader)
        else:
